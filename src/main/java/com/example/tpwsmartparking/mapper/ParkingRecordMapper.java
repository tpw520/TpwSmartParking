...
public interface ParkingRecordMapper {
    ...
    //插入一条停车记录
    @Insert("INSERT INTO parkingrecord(parking_name, number_plate,inDate_time,out_date_time,money,status) VALUES(#{parkingName},#{numberPlate},#{inDateTime},#{outDateTime},#{money},#{status})")
    ...
    //更新停车记录
    @Update("update parkingrecord set parking_name=#{parkingName}, number_plate=#{numberPlate},inDate_time=#{inDateTime},out_date_time=#{outDateTime}, money=#{money} ,status=#{status} where number_plate=#{numberPlate} ")
